...
echo "🚀 Step 1: Installing Android SDK components in Termux..."
pkg install android-sdk android-tools -y || pkg install android-sdk-tools -y || true

echo "🔍 Step 2: Auto-detecting real Android SDK location..."
SDK_PATH=""
if [ -d "$PREFIX/share/android-sdk" ]; then
    SDK_PATH="$PREFIX/share/android-sdk"
elif [ -d "$PREFIX/lib/android-sdk" ]; then
    SDK_PATH="$PREFIX/lib/android-sdk"
elif [ -d "$HOME/android-sdk" ]; then
    SDK_PATH="$HOME/android-sdk"
fi

if [ -z "$SDK_PATH" ]; then
    echo "⚙️ Creating fallback SDK directory..."
...
echo "📝 Step 3: Setting local.properties and Environment variables..."
mkdir -p android
...
echo "📁 Step 4: Preparing Separate Output Folder (/sdcard/HyperEditsPro_Official)..."
mkdir -p /sdcard/HyperEditsPro_Official

echo "🌐 Step 5: Generating Website Landing Page..."
cat << 'WEB_EOF' > /sdcard/HyperEditsPro_Official/index.html
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
echo "🔨 Step 6: Building Web Assets & Compiling Android Project..."
npm run build
...
echo "🔑 Step 7: Signing & Moving APK to /sdcard/HyperEditsPro_Official..."
cp app/build/outputs/apk/debug/app-debug.apk /sdcard/HyperEditsPro_Official/HyperEditsPro_Signed.apk

echo "===================================================="
echo "👑 SUCCESS! All Errors Fixed Cleanly!"
echo "📁 Output Folder: /sdcard/HyperEditsPro_Official"

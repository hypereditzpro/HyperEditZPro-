...
console.log('🚀 Executing Master Signed APK & Website Build Engine...');

// 1. Setup Dedicated Output Folder
const targetDir = '/sdcard/HyperEditsPro_Official';
if (!fs.existsSync(targetDir)) {
  fs.mkdirSync(targetDir, { recursive: true });
}

// 2. Generate Keystore Key (if missing)
if (!fs.existsSync('my-release-key.keystore')) {
  ...
  try {
    ...
    console.log('ℹ️ Keystore creation handled cleanly.');
  }
}

// 3. Generate Neon Purple Website Page
console.log('🌐 Generating High-Conversion Landing Page (index.html)...');
...
fs.writeFileSync(`${targetDir}/index.html`, websiteHtmlContent, 'utf8');

// 4. Build Assets & Compile Release Package
console.log('⚡ Building Web Assets & Compiling Android Release Package...');
execSync('npm run build && npx cap sync android && cd android && ./gradlew assembleRelease');

// 5. Sign APK digitally & copy to target directory
...
console.log(`🌐 Website Page: ${targetDir}/index.html`);
console.log('====================================================');
